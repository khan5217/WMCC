    ...
    >
      ...
      <div className="bg-gray-900 text-cricket-gold text-xs py-1.5 px-4">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <span className="hidden sm:block text-gray-400">contact@wmccmk.com | +44 7000 000000</span>
        </div>
      </div>
      ...
      {/* Mobile menu — scrollable, capped at remaining screen height */}
      {mobileOpen && (
        <div className="lg:hidden bg-white border-t border-gray-100 shadow-lg overflow-y-auto overscroll-contain"
          style={{ maxHeight: 'calc(100dvh - 96px)' }}>
          <nav className="max-w-7xl mx-auto px-4 pt-3 pb-10 space-y-1">
            {NAV_LINKS.map((link) =>
              link.children ? (
                <div key={link.label}>
                  ...
                  {link.children.map((child) => (
                    <Link
                      key={child.href}
                      href={child.href}
                      className="block px-3 py-3 rounded-md text-gray-700 hover:bg-green-50 hover:text-cricket-green min-h-[44px] flex items-center"
            ...
            {/* Auth buttons — always visible at the bottom */}
            <div className="pt-4 pb-2 border-t border-gray-100 space-y-3">
              {user ? (
                <>
                  <Link href="/members"
                    className="flex items-center justify-center px-4 py-4 text-gray-700 hover:bg-green-50 rounded-xl border border-gray-200 font-medium min-h-[52px]"
                    onClick={() => setMobileOpen(false)}>
                    My Profile
                  </Link>
                  <button
                    onClick={() => { handleLogout(); setMobileOpen(false) }}
                    className="w-full flex items-center justify-center px-4 py-4 text-red-600 hover:bg-red-50 rounded-xl border border-red-100 font-medium min-h-[52px]">
                    Sign Out
                  </button>
                </>
              ) : (
                <>
                  <Link href="/members/login"
                    className="flex items-center justify-center px-4 py-4 text-center border-2 border-cricket-green text-cricket-green rounded-xl font-semibold min-h-[52px] text-base"
                    onClick={() => setMobileOpen(false)}>
                    Members Login
                  </Link>
                  <Link href="/membership"
                    className="flex items-center justify-center px-4 py-4 text-center bg-cricket-green text-white rounded-xl font-semibold min-h-[52px] text-base active:bg-cricket-dark"
                    onClick={() => setMobileOpen(false)}>
                    Join the Club
                  </Link>
                </>
